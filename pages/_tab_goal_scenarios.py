...
from core.data_loader import DEFAULT_GUARDRAIL_THRESHOLDS
from app.config import DEFAULT_MONTH_DAYS, DEFAULT_DAYS_ELAPSED
...
def _run_scenarios(current: dict, goal_direction: str, gradients: List[float]):
    ...
    # Baseline
    _, _, t1_base, t2_base = run_pipeline_with_params(current)
    results.append({"label": "基线", "t1": t1_base, "t2": t2_base, "params": None})

    # 3 gradient scenarios
    for sc in scenarios:
        sc_params = {**current, **sc}
        _, _, t1_sc, t2_sc = run_pipeline_with_params(sc_params)
    ...
    for i, (col, res) in enumerate(zip(cols, results[1:])):
        with col:
            if res["params"] and st.button(
                ...
                adoption = build_adoption_params(res["params"], current)
