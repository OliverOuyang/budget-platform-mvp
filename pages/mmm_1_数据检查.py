...
with st.sidebar:
    st.header("📂 数据来源")
    # 检测 V01 是否已有上传数据
    _v01_data = st.session_state.get("uploaded_data")
    _data_options = ["使用 Mock 数据", "上传 Excel/CSV"]
    if _v01_data and "df_raw1" in _v01_data:
        _data_options.append("使用 V01 已上传数据")

    data_source = st.radio("选择数据来源", _data_options)
    if data_source == "上传 Excel/CSV":
        ...
        if uploaded:
            ...
            df = load_mock_data()
    elif data_source == "使用 V01 已上传数据":
        df = _v01_data["df_raw1"]
        st.success(f"✅ 已使用 V01 数据：{_v01_data.get('file_name', '未知文件名')}")
    else:
        df = load_mock_data()
